...
//Tried the database coding, might still need some fine tuning
//database loading and trying to connect to already created database
function loadDatabase() {
  try {
    const doc = yaml.safeLoad(fs.readFileSync('./scoopdatabase.yml', 'utf8'));
    database = doc;
  } catch (e) {
    console.log(e);
  }
...
//database save function, if error - logs to console
function saveDatabase() {
  try {
    let doc = yaml.safeDump(database);
    fs.writeFileSync('./scoopdatabase.yml', doc, 'utf8');
  } catch (e) {
    console.log(e);
  }
};

//Creating a Comment to an Article
...
//User creation if not already on file
